...
class vizeon_Mega_Menu {
	...
	function menu_wp_edit_nav_menu_walker($class, $menu_id){
		return 'vizeon_Walker_Nav_Menu_Edit';
	}
...
class vizeon_Walker extends Walker {
	/**
 ...
 */
class vizeon_Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	/**
	 * Starts the list before the elements are added.
	 *
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)' , 'vizeon'), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', 'vizeon'), $item->title );
		}
		...
		<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
                	<span class="menu-megamenu-handle"><?php esc_html_e('Mega Menu', 'vizeon'); ?></span>
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php echo esc_attr__('Move up', 'vizeon'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php echo esc_attr__('Move down', 'vizeon'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php echo esc_attr__('Edit Menu Item', 'vizeon'); ?>" href="<?php
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            	...
            	<div class="menu-megamenu">
                	<h4 class="menu-megamenu-title"><?php esc_html_e( 'Mega Menu Setting', 'vizeon' ); ?></h4>
                	<p class="menu-megamenu-enable">
                    	<label for="edit-menu-item-megamenu-<?php echo esc_attr($item_id); ?>">
                            ...
                            <?php esc_html_e( 'Mega Menu Enabled', 'vizeon' ); ?>
                        </label>
                    </p>

                    <p class="menu-megamenu-direction">
                    
                        <label> <?php echo esc_html__( 'Choose MegaMenu', 'vizeon' ); ?> <br /></label>
                    ...
                    <p class="menu-megamenu-megaicon">
                        <label>
                            <?php esc_html_e( 'Icon', 'vizeon' ); ?> <a href="<?php echo is_ssl() ? 'https' : 'http'; ?>://fortawesome.github.io/Font-Awesome/icons/" target="_bank"><?php esc_html_e('(icons in Font Awesome)', 'vizeon'); ?></a> <br />
                    ...
                    <p class="menu-megamenu-megacolumns">
                        <label>
                           <?php 
                            ...
                            echo esc_html__( 'Number columns', 'vizeon' ); ?><br />
                        	...
                        	<p><?php  echo esc_html__( 'Number columns of submenu(support 1,2,4,6 columns)', 'vizeon' ); ?></p>
                        </label>
                    </p>
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Navigation Label' , 'vizeon'); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Title Attribute' , 'vizeon'); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab' , 'vizeon'); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'CSS Classes (optional)' , 'vizeon'); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)' , 'vizeon'); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Description' , 'vizeon'); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'vizeon'); ?></span>
					</label>
				</p>

				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php esc_html_e( 'Move' , 'vizeon'); ?></span>
						<a href="#" class="menus-move-up"><?php esc_html_e( 'Up one' , 'vizeon'); ?></a>
						<a href="#" class="menus-move-down"><?php esc_html_e( 'Down one' , 'vizeon'); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php esc_html_e( 'To the top' , 'vizeon'); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', 'vizeon'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php echo esc_html__( 'Remove' , 'vizeon'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'vizeon'); ?></a>
				</div>
...
$vizeon_megamenu = new vizeon_Mega_Menu();
$vizeon_megamenu->init_mega_menu();
