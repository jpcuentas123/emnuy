...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_home() ) {
		...
		$output_html .= $prepend . $before . esc_html__( 'Products tagged &ldquo;', 'vizeon' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			$output_html .= $before . '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html__( 'Search results for &ldquo;', 'vizeon' ) . get_search_query() . '&rdquo;' . $after;
		...
		$output_html .= $before . esc_html__( 'Error 404', 'vizeon' ) . $after;
		...
		$output_html .= $before . esc_html__( 'Search results for &ldquo;', 'vizeon' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			$output_html .= $before . esc_html__( 'Posts tagged &ldquo;', 'vizeon' ) . single_tag_title( '', false ) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata( $author );
		$output_html .= $before . esc_html__( 'Author:', 'vizeon' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) ) {
		$output_html .= ' (' . esc_html__( 'Page', 'vizeon' ) . ' ' . get_query_var( 'paged' ) . ')';
	}
